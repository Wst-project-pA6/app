...
describe('StudentsController', () => {
  ...
  it('delegates every operation and exposes exact permissions', async () => {
    ...
    const dto = { userId: '22222222-2222-4222-8222-222222222222', studentNumber: 'STU-001' } as never;
    ...
    const handler = (name: string): Function => Reflect.get(StudentsController.prototype, name) as Function;
  ...
  it('validates student UUID path parameters', async () => {
    await expect(new ParseUUIDPipe().transform('not-a-uuid', { type: 'param', data: 'studentId', metatype: String })).rejects.toBeInstanceOf(BadRequestException);
  });
});
