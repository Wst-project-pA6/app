...
describe('CoursesController', () => {
  ...
  it('delegates every operation and exposes exact permissions', async () => {
    ...
    const controller = new CoursesController(service as never);
    ...
    const handler = (name: string): Function => Reflect.get(CoursesController.prototype, name) as Function;
  ...
  it('validates course UUID path parameters', async () => {
    await expect(new ParseUUIDPipe().transform('not-a-uuid', { type: 'param', data: 'courseId', metatype: String })).rejects.toBeInstanceOf(BadRequestException);
  });
});
