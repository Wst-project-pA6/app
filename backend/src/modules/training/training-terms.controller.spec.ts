...
describe('TrainingTermsController', () => {
  ...
  it('delegates every operation and exposes exact permissions', async () => {
    ...
    const controller = new TrainingTermsController(service as never);
    ...
    const handler = (name: string): Function => Reflect.get(TrainingTermsController.prototype, name) as Function;
  ...
  it('validates term UUID path parameters', async () => {
    await expect(new ParseUUIDPipe().transform('not-a-uuid', { type: 'param', data: 'termId', metatype: String })).rejects.toBeInstanceOf(BadRequestException);
  });
});
